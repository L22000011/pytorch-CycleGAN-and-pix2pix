...
class CycleGANModel(BaseModel):
    """
    CycleGAN model with optional Edge loss.
    """
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionEdge = torch.nn.L1Loss()  # Edge loss
    ...
    def backward_G(self):
        ...
        lambda_edge = getattr(self.opt, "lambda_edge", 0.0)
        ...
        # Edge loss
        if lambda_edge > 0:
            edge_real_A = self.real_A[:, :, 1:-1, 1:-1] - self.real_A[:, :, :-2, :-2]  # 简单Sobel/差分边缘示例
            edge_fake_B = self.fake_B[:, :, 1:-1, 1:-1] - self.fake_B[:, :, :-2, :-2]
            self.loss_edge_A = self.criterionEdge(edge_fake_B, edge_real_A) * lambda_edge

            edge_real_B = self.real_B[:, :, 1:-1, 1:-1] - self.real_B[:, :, :-2, :-2]
            edge_fake_A = self.fake_A[:, :, 1:-1, 1:-1] - self.fake_A[:, :, :-2, :-2]
        ...
        self.loss_G = (self.loss_G_A + self.loss_G_B +
                       ...
                       self.loss_edge_A + self.loss_edge_B)
